...
export const roomsEvents = (
  ...
  socket.on("rooms:joinRoom", async (room: IRoom) => {
    ...
    io.emit("rooms:newRoomResponse", userRooms);
  });

  socket.on("rooms:leaveRoom", async ({ room, sender }) => {
    try {
      socket.leave(room);
      ...
      if (currentRoom && currentRoom.participants.length === 0) {
        ...
        console.log(`Room ${room} deleted as there were no participants left`);
      }
      ...
      socket.emit("rooms:leaveRoomResponse", rooms);
    } catch (error: any) {
      console.error("Error leaving room:", error);
      socket.emit("error", { message: `${error.message}, room: ${room}` });
    }
  });
  ...
  socket.on("rooms:removeUser", async ({ senderId, roomId }) => {
    try {
      ...
      if (!sender || (sender.role !== "admin" && sender.role !== "owner")) {
        socket.emit("error", {
          message: "Permission denied. Only admins or owners can remove users.",
        });
        return;
      }
  ...
  socket.on(
    "rooms:promoteUser",
    async ({ senderId, userId, roomId, newRole }) => {
      try {
        const room = await RoomModel.findById(roomId, { isDeleted: false })
          .populate({
            path: "participants.user",
  ...
  socket.on("rooms:getRoomData", async (roomId: string) => {
    ...
    if (room) {
      socket.emit("rooms:getRoomDataResponse", room);
    } else {
      socket.emit("error", { message: "Room Not Found" });
    }
  });

  socket.on("rooms:newRoom", async (data: any) => {
    ...
    io.emit("rooms:newRoomResponse", rooms);
  });

  socket.on("rooms:promoteToAdmin", async ({ roomId, userId }) => {
    const room = await RoomModel.findById(roomId, { isDeleted: false });
